...
void Dictionary::initNgramsRelearn(int32_t offset) {
...
void Dictionary::readFromFileForRelearn(std::istream& in, int32_t offset) {
  std::string word;
  int64_t minThreshold = 1;
  while (readWord(in, word)) {
    add(word);
    ...
    if (size_ > 0.8 * MAX_VOCAB_SIZE) {
      minThreshold++;
      threshold(minThreshold, minThreshold);
    }
  }
  threshold(args_->minCount, args_->minCountLabel);
  initNgramsRelearn(offset);
  if (args_->verbose > 0) {
    std::cerr << "\rRead " << ntokens_ / 1000000 << "M words" << std::endl;
    std::cerr << "Number of words:  " << nwords_ << std::endl;
  }
