...
void FastText::relearnThread(int32_t threadId) {
    ...
    while (tokenCount_ < args_->epoch * ntokens) {
      ...
      if (args_->model == model_name::sg) {
        ...
        skipgramRelearn(model, lr, line);
      } else {
          throw std::invalid_argument("Only skipgram model may be relearned");
      }
...
void FastText::loadVectorsForRelearn() {
  std::shared_ptr<Matrix> mat; // new matrix with pretrained vectors
  mat = std::make_shared<Matrix>(dict_->nwords() + args_->bucket, args_->dim);
  mat->uniform(1.0 / args_->dim);
  for (size_t i = 0; i < input_->rows(); i++) {
    for (size_t j = 0; j < dim; j++) {
      mat->at(i, j) = input_->at(i, j);
    }
  }
  input_ = mat;

  std::shared_ptr<Matrix> mat_out_;
  mat_out_ = std::make_shared<Matrix>(dict_->nwords(), args_->dim);
  mat_out_->zero();
  for (size_t i = 0; i < output_->rows(); i++) {
    for (size_t j = 0; j < dim; j++) {
      mat_out->at(i, j) = output_->at(i, j);
    }
  }
  output_ = mat_out_;
}
...
void FastText::relearn(const Args args) {
    args_ = std::make_shared<Args>(args);
    if (args_->model_filename == "") {
        throw std::invalid_argument("Cannot load model for relearning");
    }
    FastText fasttext;
    fasttext.loadModel(args_->model_filename);
    fasttext.dict_->initNgrams();
    ...
    fasttext.dict_->readFromFileForRelearn(ifs, fasttext.dict_->size_);
    ifs.close();

    loadVectorsForRelearn();
    startThreads();

    model_ = std::make_shared<Model>(input_, output_, args_, 0);
    model_->setTargetCounts(fasttext.dict_->getCounts(entry_type::word));
